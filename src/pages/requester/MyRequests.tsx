...
export default function MyRequests({ requests, setActiveTab, setSelectedRequest }: MyRequestsProps) {
	...
	const filteredRequests = requests.filter(req => {
		...
		if (endDate) {
			matchesDate = matchesDate && req.createdDate <= endDate;
		}
			...
			<div className="bg-white border border-zinc-200/50 rounded-3xl shadow-sm overflow-hidden p-1">
				{filteredRequests.length === 0 ? (
					<div className="text-center py-16">
						...
						<p className="text-xs text-zinc-650 font-light mt-1">Refine active filters or create a new request.</p>
					</div>
				) : (
					<div className="overflow-x-auto flex flex-col justify-between">
						...
						<Pagination
							...
							itemNamePlural="testing requests"
						/>
					</div>
				)}
			</div>
